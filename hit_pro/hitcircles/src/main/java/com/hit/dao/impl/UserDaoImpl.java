...
public class UserDaoImpl implements UserDao {
    ...
    public UserInfoResponse getUserInfoById(int userId) {
        try {
            ...
            String sql = "SELECT ub.user_id AS id, ub.username, ua.avatar_url AS avatar, is_admin " +
    ...
    public UserInfoResponse getUserBaseInfoById(int userId) {
        try {
            QueryRunner qr = new QueryRunner(DruidUtils.getDataSource());
            String sql = "SELECT u.user_id, u.username, a.avatar_url, is_admin FROM user_base u JOIN user_avatar a ON u.user_id = a.user_id WHERE u.user_id = ?";
            return qr.query(sql, new BeanHandler<>(UserInfoResponse.class), userId);
        } catch (SQLException e) {
            e.printStackTrace();
            return null;
